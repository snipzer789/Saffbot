...
async function UpdateDataToDB(DB, UsageCount, Price, YoutubeapiKey, GuildId, DiscordChannel, YoutubeChannelId, LastStreams , DiscordUpdateType) {
  try {
    ...
    //}
    if(Price == -100){
      await Collection.updateMany({UsageCount: {$gt: 0}}, { $set: { UsageCount: 0 }})
    }
...
async function LoadDataFromDB(DB) {
  try {
    ...
    if (DB == 'discordChannels'){
      ...
      array.push(cursor.DiscordChannelId,cursor.guildId, cursor.YoutubeId, cursor.LastStream)
      console.log(array)
      return array
    }
...
const YoutubeChannelIds = async(YoutubeChannelUrl) => {
  try {
    ...
    const response = await axios.get(`${YoutubeUrlApicall}/search?part=id,snippet&maxResults=1&type=channel&q=${YoutubeChannelUrl}&key=${YoutubeApiInuse[0]}`)
